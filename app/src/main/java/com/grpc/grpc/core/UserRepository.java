...
public final class UserRepository {
    ...
    private static Map<String, Object> buildProfileSeed(@Nullable DocumentSnapshot existing,
        ...
        if (session != null) {
            ...
            maybePutBoolean(existing, out, "canMessage", session.canMessage);
            maybePutBoolean(existing, out, "canMap", session.canMap);
            maybePutBoolean(existing, out, "bugReport", session.canBugReport);
        }

        // Seed contract visibility and messaging/maps flags based on role, without overwriting existing values.
        String roleForFlags;
        ...
        // canMessage / canMap / bugReport: seed when missing (from session or role default).
        boolean defaultCanMessage = "super_admin".equals(roleForFlags);
        boolean defaultCanMap = "super_admin".equals(roleForFlags);
        boolean defaultBugReport = "super_admin".equals(roleForFlags) || "admin".equals(roleForFlags);
        maybePutBoolean(existing, out, "canMessage", session != null ? session.canMessage : defaultCanMessage);
        maybePutBoolean(existing, out, "canMap", session != null ? session.canMap : defaultCanMap);
        maybePutBoolean(existing, out, "bugReport", session != null ? session.canBugReport : defaultBugReport);
