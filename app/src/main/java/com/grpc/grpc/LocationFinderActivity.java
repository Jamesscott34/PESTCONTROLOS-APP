...
public class LocationFinderActivity extends AppCompatActivity {
    ...
    private void buildTechRows() {
        ...
        for (String tech : TECHS) {
            ...
            open.setOnClickListener(v -> openTechLocation(tech));
            card.addView(open);

            container.addView(card);

            // Seed from offline cache immediately
            updateDetailsFromCache(tech);
        }
    }
    ...
    private void updateDetailsFromCache(String tech) {
        TextView details = findDetailsView(tech);
        if (details == null) return;

        String cached = LocationSharing.getCachedLastLocation(this, tech);
    ...
    private void openTechLocation(String tech) {
        String cached = LocationSharing.getCachedLastLocation(this, tech);
        if (TextUtils.isEmpty(cached)) {
            Toast.makeText(this, "No location saved for " + tech, Toast.LENGTH_SHORT).show();
            return;
        }
        ...
        Toast.makeText(this, "No usable location for " + tech, Toast.LENGTH_SHORT).show();
    }
